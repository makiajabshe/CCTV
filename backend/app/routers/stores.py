...
async def list_stores(session: Annotated[AsyncSession, Depends(get_session)], user: User) -> list[StoreOut]:
    ...
    return [StoreOut(store_id=s.id, tenant_id=s.tenant_id, name=s.name, timezone=s.timezone) for s in rows]
...
async def devices(store_id: UUID, session: Annotated[AsyncSession, Depends(get_session)], user: User) -> list[DeviceOut]:
    ...
    return [DeviceOut(device_id=d.id, name=d.name, is_active=d.is_active, last_event_at=d.last_seen_at,
                      ...
                      agent_version=d.hb_agent_version) for d in rows]
