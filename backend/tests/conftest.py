...
async def _seed(url: str) -> Fixture:
    ...
    async with db.sessionmaker() as s:
        ...
        s.add_all([ua, ub, u_none, u_off])
        await s.flush()
        s.add_all([TenantMembership(user_id=ua.id, tenant_id=t1.id), TenantMembership(user_id=ub.id, tenant_id=t2.id),
                   TenantMembership(user_id=u_off.id, tenant_id=t1.id)])
        await s.commit()
