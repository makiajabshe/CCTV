...
def test_ingest_updates_last_seen(client, fx, auth_a):
    ...
    assert dev_a["last_seen_at"] is not None
    assert dev_a["api_key_prefix"] == fx.keys["a"][:12]
    assert fx.keys["a"] not in str(devs)
